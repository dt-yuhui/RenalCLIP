...
class ImageCaptionDataset(Dataset):
    def __init__(self, args, hospital='internal', split='train', stage=2):
        super().__init__()
        ...
        self.split = split
        self.pre_processed_type = "one_kidney_tc_v3" if stage==2 else "one_kidney_kc_v3"
        self.seed = args.seed
        ...
        self.keys = ["img"]
        self.transforms = get_transforms(mode=split, args=args, keys=self.keys, resize=args.resize)
        self.stage = stage
    ...
    def read_per_dataset(self, dataset_name, dataset_attr):
        ...
        if self.stage == 1:
            _train_filenames = caption_data_split['pretrain_data']['train_set']
            _test_filenames = caption_data_split['pretrain_data']['valid_set']
        else:
            if self.hospital == 'internal':
                _train_filenames = caption_data_split['downstream_data_internal']['train_set']
                _test_filenames = caption_data_split['downstream_data_internal']['test_set']
            else:
                _train_filenames = caption_data_split['downstream_data_external'][self.hospital]['train_set']
                _test_filenames = caption_data_split['downstream_data_external'][self.hospital]['test_set']
    ...
    def load_text_data(self, dataset_name, dataset_root):
        for idx, row in enumerate(self.df.itertuples()):
            ...
            if self.stage == 2:
                tumor_side = getattr(row, 'tumor_side')
                if tumor_side == 'L':
                ...
                left_kidney_findings = preprocess_text(split_findings.get('Left Kidney', ''))
                left_kidney_impression = preprocess_text(split_impression.get('Left Kidney', ''))
                right_kidney_findings = preprocess_text(split_findings.get('Right Kidney', ''))
                right_kidney_impression = preprocess_text(split_impression.get('Right Kidney', ''))
                ...
                left_info_dict = {
                    'patient_id': patient_id,
                    'tumor_side': 'L',
                    'kidney_findings': left_kidney_findings,
                    'kidney_impression': left_kidney_impression,
                }
                right_info_dict = {
                    'patient_id': patient_id,
                    'tumor_side': 'R',
                    'kidney_findings': right_kidney_findings,
                    'kidney_impression': right_kidney_impression,
                }
